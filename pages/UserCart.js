...
function UserCart() {
          ...
          if (currentDate2 >= startDate && currentDate2 <= endDate) {
            if (couponDetail.min_buy <= cartTotal) {
              ...
              if (couponDetail.discount_type.toLowerCase() === "percentage") {
                // discountedAmount = ((parseFloat(couponDetail.discount)) / 100) * Number(subTotal)
                // subTotal - calculatedDis
                discountedAmount =
                  (parseFloat(couponDetail.discount) / 100) *
                  Number(subTotal) || 0;
                // discountedAmount = ((parseFloat(couponDetail.discount)) / 100) * Number(subTotal) > parseFloat(couponDetail.max_discount) ? parseFloat(couponDetail.max_discount) : ((parseFloat(couponDetail.discount)) / 100) * Number(subTotal)
                // console.log(discountedAmount, "<discountedAmount<<<<<", couponDetail);
                setDiscountedPrice(subTotal - discountedAmount);
                setGrandTotal(subTotal - discountedAmount + shipingPrice);
  ...
  useEffect(() => {
    ...
    // console.log(shipping_cost, "<<<<shipping_cost"  ,data,removeDuplicatesByProductId(data));
    setShipingPrice(
      totalA > 2000
        ? 0
        : shipping_cost
  ...
  useEffect(() => {
    // console.log(getCartListData.getCartListData, "<<<<<<getCartListData.getCartListData");
                            ...
                            <div className="row">
                              <div className="col-lg-3 col-4">
                                <div className="cart-images ">
                                  <a
                                    href={`/product-details?id=${item?.product_id
              ...
              <div className="row mb-3">
                <div className="col-lg-4">
                  <div className="copen-apply">
                    ...
                    {/* <p>Discounted Price: {discountedPrice} Rupees</p> */}
                    {/* {console.log(errorMessage, "<<<<<<errorMessage", couponPrice)} */}
                  </div>
                  {couponPrice && (
